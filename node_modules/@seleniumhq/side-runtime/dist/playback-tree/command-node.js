    ...
    */
    ...
    retryCommand(execute, timeout) {
        return new Promise((res, rej) => {
            ...
            if (timeLimit <= 0) {
                return rej(new Error(`Operation timed out running command ${this.command.command}:${this.command.target}:${this.command.value}`));
            }
            const expirationTimer = setTimeout(() => {
                rej(new Error(`Operation timed out running command ${this.command.command}:${this.command.target}:${this.command.value}`));
            }, timeLimit);
            execute()
                .then((result) => {
